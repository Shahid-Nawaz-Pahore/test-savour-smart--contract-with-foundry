...
import "forge-std/console.sol"; // Optional: for Foundry console logging



contract Savor is ERC20, ERC20Burnable, Pausable, Ownable, ReentrancyGuard {
    ...
    function preSale_2_Listing(uint256 endTime) external onlyOwner {
    ...
    function preSale_3_Listing(uint256 endTime) external onlyOwner {
    ...
    function startTimee(uint256 _startTime) external onlyOwner {
        startTime = _startTime;
    }

    function setTime(uint256 presale, uint256 time) public onlyOwner {
        if (presale == 1) {
